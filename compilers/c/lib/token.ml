...
let split_field str pos =
  match String.index_from_opt str pos ' ' with
  | Some next ->
      (String.sub str pos (next - pos), next + 1)
  | None ->
      (String.sub str pos (String.length str - pos), String.length str)
...
let deserialize line =
  ...
  let kind =
    ...
    | "IDENTIFIER" ->
        Identifier payload
    | "CONSTANT_INT" ->
        ConstantInt (int_of_string payload)
    | other ->
        failwith ("Unknown token kind: " ^ other)
  in
